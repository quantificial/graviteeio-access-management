...
public class MongoResourceSetRepository extends AbstractManagementMongoRepository implements ResourceSetRepository {
    ...
    public static final String COLLECTION_NAME = "uma_protected_resource_set";
    ...
    public Single<List<ResourceSet>> findByDomainAndClientAndUser(String domain, String client, String user) {
        return Observable.fromPublisher(resourceSetCollection.find(and(eq(FIELD_DOMAIN, domain), eq(FIELD_CLIENT, client), eq(FIELD_USER, user)))).map(this::convert).collect(ArrayList::new, List::add);
    }
